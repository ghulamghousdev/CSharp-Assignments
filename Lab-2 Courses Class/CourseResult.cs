...
namespace Student
{
    class CourseResult
    {
        ...
        public CourseResult(string name, string id, int crd, int marks, int sem)
        {
            if (!isCourseIDValid(id)) //Validating Course ID
            {
                Console.WriteLine("CourseID is invalid");
                Environment.Exit(0);
            }
            if (!isCreditHoursValid(crd)) //Validating Credit hours
            {
                Console.WriteLine("Credit Hours are invalid");
                Environment.Exit(0);
            }
            if (!isSemesterValid(sem)) //Validating Semesters
            {
                Console.WriteLine("Semester is invalid");
                Environment.Exit(0);
            }
            if (!isMarksValid(marks)) //Validating marks
            {
                Console.WriteLine("Marks are invalid");
                Environment.Exit(0);
            }
            if (!isCourseTitleValid(name)) //Validating course name
            {
